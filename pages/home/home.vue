<template>
	<view class="home1">
		...
		<view class="center">
			<view class="text-sl message" @tap="toMsg"><text class="cuIcon-notice " :class="messageColor ? 'text-blue' : 'text-red'"
				 v-if="showMessage"></text></view>
			<view class="top">
			...
			<view class="grid flex-wrap text-center col-4 padding contentView solid-bottom">
				<view class="padding-bottom-xs round" v-for="(item, index) in navs" :key="index" @tap="change2(index)">
			...
			<view class="tui-rolling-news solid-bottom">
				...
				<swiper vertical autoplay circular interval="3000" class="tui-swiper">
					<swiper-item v-for="(item, index) in newsList" :key="index" class="tui-swiper-item">
						<view class="tui-news-item" @tap="detail(item)">{{ item.title }}</view>
					</swiper-item>
				</swiper>
			</view>
			<view class="grid flex-wrap text-center col-4 padding contentView solid-bottom">
				<view class="padding-bottom-xs round" v-for="(item, index) in navs2" :key="index" @tap="change(index)">
	...
	export default {
		...
		data() {
			return {
				...
				navs: [{
						id: 1,
						icon: '../../static/img/nj.png',
						title: '代办年检',
						page: '/pages/home/errorPage/errorPage',
						badge: 0
					},
					/* {
						id: 2,
						icon: '../../static/img/wz.png',
						title: '违章查询',
						page: '/pages/home/errorPage/errorPage',
						badge: 0
					}, */
		...
		methods: {
			detail(e) {
				uni.navigateTo({
					url: 'newsDetail?map=' + JSON.stringify(e),
					success: res => {},
					fail: () => {},
					complete: () => {}
				});
			...
			change2(e) {
				if (!this.hasLogin) {
					...
					uni.navigateTo({
						url: this.navs[e].page + '?title=' + this.navs[e].title,
						success: res => {},
						fail: () => {
							uni.showToast({
								title: '开发中！！',
			...
			getHeadlinesList() {
				let param = {};
				this.$http.post(this.$api.getHeadlinesList, param, 1).then(e => {
					if (e.code == 200) {
						this.newsList = e.data
					} else {
						let p = {
							title: '暂无'
						}
						this.newsList.push(p)
					}
				});
			},
			getListByOrderNum() {
			...
			if (!this.hasLogin) {
				...
				uni.login({
					...
					success: res => {
						...
						this.$http.post(this.$api.hasBound, param).then(e => {
							if (e.code == 200) {
								if (e.data.user != '' && e.data.user != null) {
