<template>
	<view class="home">
		...
		<view class="center">
			...
			<view class="padding solid-bottom" v-else @tap="toSetDelivery">
				<view class="flex ">
					...
					<view class="flex-sub radius">
						<view class="cu-tag line-orange radius" v-if="info.isDefault == 1">默认地址</view>
					</view>
					<view class="cuIcon-right"></view>
				</view>
				<view>
			...
			<view v-if="isShow">
				<view class="cu-form-group solid-bottom" v-if="map.quote.forceTax==0 || map.quote.forceTax==1">
					<view class="title">商业险优惠</view>
					<view class="flex align-center text-right">{{ commission.bizPercentage }}%</view>
				</view>
				<view class="cu-form-group solid-bottom" v-if="map.quote.forceTax==2 || map.quote.forceTax==1">
		...
		<view class="head padding text-center align-center justify-center solid-bottom solids-top margin-top"><button class="cu-btn bg-blue lg"
			 @tap="next">提交订单</button></view>
	</view>
</template>
