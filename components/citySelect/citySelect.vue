	...
	methods: {
		...
		loadCity(longitude, latitude) {
			...
			wx.request({
				...
				data: {
					key: 'bec147fd9b0a1535bc72c63cb59e3177',
				...
				success: function(res) {
					...
					_self.position=city.substring(0,city.length-1)
					_self.$emit('back_city', _self.position);
			...
			let countdown = setInterval(() => {
				...
				uni.getLocation({
					...
					success: function(res) {
						...
						that.loadCity(that.longitude,that.latitude)
					}
				});
				if (that.seconds <= 0) {
					that.seconds = 1;
